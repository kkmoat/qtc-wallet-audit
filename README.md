...
本次更新仅在已有文件范围内同步 Worker 加载握手、处理阶段消息、超时与取消、过期会话拒绝、恢复保存失败后的锁定，以及对应钱包界面与错误文案。原有 `history_add` 签名协议仍在选中代码范围内；没有加入市场工作台、后台鉴权、数据库写入实现、历史成交数据或生产配置。现有本地测试覆盖钱包回归和随机输入行为，不代表已经验证整站弹窗挂载、完整服务器流程或线上部署。
