...
| 浏览器向外发送什么 | [client.mjs](lib/market/client.mjs) / [wallet.ts](lib/market/wallet.ts) | 市场请求发送公开地址、公钥、操作内容和签名。该市场 API 客户端仅对未显式提供旧审核密码的审核提交启用同源 Cookie；通用请求、签名挑战和普通操作省略 Cookie。余额查询向官方索引服务发送公开地址。选中的调用路径没有上传钱包助记词、密文备份或本地解锁密码。 |

管理员访问密码、旧审核密码与用户的本地钱包解锁密码是不同的东西。后台登录会通过 HTTPS 将访问密码发送服务器验证并获得 HttpOnly 会话 Cookie；新版审核复用此会话，不在每次审核中再次发送访问密码。`client.mjs` 仍兼容调用方显式传入旧 `reviewPassword`：只随审核提交发送，不进入钱包签名载荷或 Worker。登录界面、Cookie 签发和服务端权限校验不在这个公共包中，不能仅凭本包验证它们；也不能声称“任何密码都不会发送服务器”。公开签名协议中的平台收款地址、手续费等是用户已经能看到的交易条款，不是私钥。
