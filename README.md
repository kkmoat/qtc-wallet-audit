...
对应市场源码版本：`56b5d98a4797ff7168144ecf0e138f2273ae0c05`（2026-09-11）。这是用于标识来源的版本号，原市场仓库保持私密；公众可以直接查看本仓库的选中源码。每个原样复制或从源码包解出的文件都有 [SHA-256 清单](snapshot.json)。本次线上资产比对状态见 [验证记录](VERIFICATION.md)。这不是第三方安全审计报告，也不是“绝对安全”证书。
...
测试覆盖生成、解密恢复、错误密码、篡改备份、域隔离签名、锁定、随机接口调用及拒绝意外网络请求。生成的测试钱包只存在于测试进程内，不输出助记词、私钥、密码或地址。它们不能代替实际浏览器、整站或第三方独立审计。
...
本次更新仅在已有文件范围内同步 `history_add` 补录成交操作的签名字段校验、审核密码请求类型和错误文案。它让审阅者检查钱包签署的数量、价格、时间及记录编号；没有加入后台鉴权、数据库写入实现、历史成交数据或生产配置。现有本地测试覆盖钱包回归和随机输入行为，不代表已经验证该补录操作的完整服务器流程。
